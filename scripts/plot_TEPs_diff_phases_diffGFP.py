...
from src.visualization.TEPs_different_phases import plot_TEPs_per_phases_GFP
...
def plot_teps(filename):
    ...
    fig = plot_TEPs_per_phases_GFP(TEP_pre - TEP_post, TEP_im - TEP_post, TEP_post - TEP_post, time, ms_to_samples)
    # fig = plot_TEPs_per_phases_GFP(TEP_pre - TEP_pre, TEP_pre - TEP_im , TEP_pre - TEP_post, time, ms_to_samples)

    fl = os.path.basename(filename)
    fig.suptitle(fl)
    
    output_filename = os.path.join(r"./results/TEPs_differentPhases/post_diffGFP", "POSTdiffGFP_"+os.path.splitext(fl)[0]+".png")
